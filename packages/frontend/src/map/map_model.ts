...
// ─── Road Status (used by metric chart cards) ─────────────────

/** Road status derived from saturation score — used by MetricCharts. */
export type RoadStatus = 'clear' | 'caution' | 'blocked' | 'unknown';

/**
 * Maps Saturation_Score (0–1) to RoadStatus.
 * Green (clear): < 0.6, Yellow (caution): 0.6–0.85, Red (blocked): > 0.85.
 *
 * NOTE: This is used only by the metric chart cards in CommandCenterShell.
 * The schematic OperationsMap renders the backend's `level` A/B verdict verbatim
 * and does not compute its own saturation-based status.
 */
