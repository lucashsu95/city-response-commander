...
describe("check-language-boundary", () => {
  ...
  it("FAILS for mixed languages in nested subdirectories", () => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "src", "core", "engine.ts"),
      "export {};"
    );
    fs.writeFileSync(
      path.join(pkgDir, "src", "scripts", "migrate.py"),
      "pass"
    );
  ...
  it("ignores .py files inside node_modules", () => {
    ...
    fs.writeFileSync(path.join(pkgDir, "src", "index.ts"), "export {};");
    fs.writeFileSync(
      path.join(pkgDir, "node_modules", "some-dep", "script.py"),
      "pass"
    );
