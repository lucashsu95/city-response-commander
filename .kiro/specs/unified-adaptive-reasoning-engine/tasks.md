...
## Post-review fixes (code-review after TASK-UARE-11)

A `/code-review` two-axis pass (Standards + Spec sub-agents against `main...feature/unified-adaptive-reasoning-engine`) found one critical Spec-axis gap and one Spec-axis text-fidelity gap. Both fixed in the same review round, before push:

1. **Critical — `buildDecisionCore` silently dropped all 4 UARE fields (R5 AC1/AC5).** `packages/domain/src/decision/decision_core_builder.ts`'s `structuredClone({...})` call used its own separate, hand-maintained 24-field allowlist (predating this spec) that never referenced `input.sop_matched`/`sop_authority`/`universal_principles`/`grounding_candidates`. TASK-UARE-08 only populated `DeterministicDecisionFacts`; nothing propagated those fields into the actual `DecisionCore` object that gets hashed and would be persisted — every test claiming to prove R9.1/R9.2/R9.6 asserted on `result.facts.*`, never on `buildDecisionCore`'s output, so this was invisible to the test suite. Fixed by adding the 4 fields to the `structuredClone` call. Deliberately **not** added to `canonicalDecisionPayload`'s separate §10.11a-1 hash allowlist (`canonical_core_hash.ts`) — verified this keeps `core_hash` byte-identical with/without UARE fields present (new test), matching the existing precedent that `injection_run_id`/`version`/`workflow_execution_name` are present on `DecisionCore` but excluded from the hash. New tests in `packages/domain/test/unit/decision_core_builder.test.ts` (2 new, 9/9 total pass) prove both the pass-through and the hash-stability property.

2. **R6 AC2's literal disclosure sentence was never emitted.** `requirements.md` R6 AC1's condition is exactly "`sop_matched` false AND `grounding_candidates` is empty (= R3 AC7)" — it does not distinguish "anchor unresolvable" from "alternatives filtered to nothing"; both collapse into the same case. R6 AC2 requires that case to emit the literal sentence "本事件地點資料亦無法定位可用替代路段，建議應變依通用原則執行並儘速人工確認". The original `buildUniversalDefenseSection` used a different, non-compliant generic fallback ("無可用替代路段，僅能提供不依賴具體道路之通用處置建議") for this case instead. Fixed in `packages/ai-generator/src/recommendation-generator.ts` (and mirrored in design.md §6's pseudocode) to emit the exact required sentence when `grounding_candidates.length === 0`, alongside the unconditional R4 AC6 disclosure. `packages/ai-generator/test/recommendation-generator.test.ts`'s R6 test updated to assert on the literal required sentence (5/5 pass).

Full regression after both fixes: `packages/domain` 455 pass / 11 pre-existing unrelated failures (unchanged); `packages/ai-generator` 5/5; all 5 packages typecheck clean.
